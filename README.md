...
Проект покрыт юнит-тестами (49 тестов):

| Слой      | Файл                             | Тестов | Описание                                         |
|-----------|----------------------------------|--------|--------------------------------------------------|
| Model     | `internal/model/*_test.go`       | 2      | Конвертация моделей в DTO                         |
| Service   | `internal/service/*_test.go`     | 29     | Валидация, бизнес-логика, обработка ошибок        |
| Handler   | `internal/handler/*_test.go`     | 18     | HTTP-статусы, парсинг запросов, роутинг           |

Слои тестируются изолированно через интерфейсы и моки.
